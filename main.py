...
def is_valid_submittal_item_detail_pdf(pdf_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        # Check first few pages, not just page 1
        sample_text_parts = []
        for page in reader.pages[:3]:
            sample_text_parts.append(page.extract_text() or "")
        ...
        missing_core = [m for m in required_core_markers if m not in text_lower]
        if missing_core:
            return False, (
                "PDF does not look like a 'Submittal item detail' export "
                f"(missing core marker(s): {', '.join(missing_core)})"
            )

        support_hits = sum(1 for m in supporting_markers if m in text_lower)
        if support_hits < 2:
            return False, (
                "PDF may not be a valid 'Submittal item detail' export "
                "(not enough supporting markers found)"
            )
...
def format_display_name(name: str) -> str:
    ...
    def normalise_word(word: str) -> str:
        ...
        # Leave existing all-caps / code-like tokens alone
        if re.fullmatch(r"[A-Z0-9&/.-]+", word):
            return word

        # Leave CamelCase-ish words alone
        if any(c.islower() for c in word) and any(c.isupper() for c in word):
            return word

        # Leave anything with digits alone if it already has capitals
        if any(c.isdigit() for c in word) and any(c.isupper() for c in word):
            return word

        # Only prettify plain lowercase words
        if word.islower():
            return word.capitalize()

        return word

    return " ".join(normalise_word(word) for word in cleaned.split())
